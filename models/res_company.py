...
class Company(models.Model):
    ...
    def _inverse_product_pricelist(self):
        ...
        if self.property_product_pricelist or (actual and default_for_country and default_for_country.id != actual.id):
            ...
            self.env['ir.property'].with_context(
                force_company=self._context.get('force_company', self.env.user.company_id.id)).sudo().set_multi(
    ...
    def _commercial_fields(self):
        return super(Company, self)._commercial_fields() + ['property_product_pricelist']
